        ...
        <div class="card-box">
            <div class="row pb-3">
                <div class="col-md-12 text-right">
                    <?= $this->Html->link(__('<i class="mdi mdi-plus-circle-outline"></i> เพิ่มสินค้า'), ['action' => 'add'], ['class' => 'btn btn-primary btn-rounded w-md waves-effect waves-light m-b-5', 'escape' => false]) ?>
                </div>
            </div>
            <table cellpadding="0" cellspacing="0" id="datatable" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        ?>
                        <td class="text-center"><?= number_format(h($product->price)) ?> <?php if($product->special_price != 0){ echo "<strong style='color: #000;'>/</strong> <span style='color: #dd0000;'>".number_format(h($product->special_price))."</span>"." <span>( ".number_format(h(($product->price - $product->special_price)/$product->price)*100)."% )</span>";} ?> <?= $this->Html->link(__('<i class="mdi mdi-tooltip-edit"></i>'), ['action' => 'setPrice', $product->id], $modalPrice) ?></td>
                        <?php
                            $modalQty = [
                                'data-id' => $product->id,
                                'data-name' => $product->name,
                        ...
                        ?>
                        <td class="text-center"><strong><?php if($product->qty >= 20){ echo "<span style='color: #18e000;'>".number_format(h($product->qty))."</span>"; }else{ echo "<span style='color: #dd0000;'>".number_format(h($product->qty))."</span>"; } ?></strong> <?= $this->Html->link(__('<i class="mdi mdi-tooltip-edit"></i>'), ['action' => 'setQty', $product->id], $modalQty) ?> </td>
