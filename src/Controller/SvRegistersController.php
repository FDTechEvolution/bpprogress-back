...
class SvRegistersController extends AppController {
    ...
    public function requestOtp($mobile = null) {
        ...
        $user = $this->Users->find()
                ...
                ->first();
        if ($user) {
            $data = $this->Sms->getSendOTP($user->id, $user->mobile);
            $this->responData['status'] = 200;
            $this->responData['data'] = $data;
        } else {
            $this->responData['msg'] = 'ไม่พบผู้ใช้งาน';
        }
    ...
    public function verifyOtp($ref = '', $otpCode = '') {
        $this->autoRender = false;
        $this->modifyHeader();
        $this->RequestHandler->respondAs('json');

        $result = $this->Sms->verify($ref, $otpCode);

        if ($result != null) {
            $this->responData['status'] = 200;
            $this->responData['data'] = $result;
