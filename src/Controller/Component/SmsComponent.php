...
class SmsComponent extends Component {
    ...
    public function send($mobile = '',$sms = ''){
        $this->sendSMS('0000', $mobile, $sms);
    }

    private function sendSMS($from = '0000', $to = null, $message = null) {
        $params['method'] = 'send';
    ...
    public function getSendOTP($id = '', $mobile = '') {
        ...
        if ($this->Otps->save($otp)) {
            $message = "รหัส OTP ของคุณคือ " . $otp->otp_code . " รหัสอ้างอิง " . $otp->otp_ref;
            $this->sendSMS('0000', $mobile, $message);
            return $otp_return = [$id, $otp->otp_ref];
        }
    }

    public function verify($ref = '', $otpCode = '') {
        $this->Otps = TableRegistry::get('user_otps');
        $otp = $this->Otps->find()
                ->where(['otp_ref' => $ref, 'otp_code' => $otpCode, 'isactive' => 'Y'])
                ->first();
        if ($otp) {
            $otp->isactive = 'N';
            $this->Otps->save($otp);
